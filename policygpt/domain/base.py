...
settings in one place.  Add a new file under domain/ to support a new
...
class DomainProfile:
    ...
    # Hint appended to response format for "list all X" queries.
    aggregate_response_hint: str
    # Text markers in section text that signal the section is a good source
    # for aggregate queries (e.g. "contest name" for reward, "policy scope" for enterprise).
    aggregate_positive_markers: tuple

    # ── Entity extraction ─────────────────────────────────────────────────────
    entity_categories: frozenset          # recognised category names
    entity_global_categories: frozenset   # categories applied document-wide (not section-scoped)
